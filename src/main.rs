...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let base_url =
    Url::parse("https://dl.fedoraproject.org/pub/fedora/linux/development/rawhide/Everything/x86_64/os/Packages/q/")?;
  ...
  let argv: Vec<String> = vec!["rpm".into(), "-Uvh".into()].into_iter().chain(rpm_files.into_iter()).collect();
